...
function Miner(id, login, pass, ip, payid, startingDiff, pushMessage){
    this.id = id;
    this.login = login;
    this.pass = pass;
    this.ip = ip;
...
var redisCommand=[[]];
    ...
    if (miner.payid != null) {
     redisCommands = [
        ...
        ['hset', config.coin + ':workers:' + miner.login+':'+miner.payid, 'paymentID', miner.payid]];
...
function handleMinerMethod(method, params, ip, portData, sendReply, pushMessage){
    ...
    switch(method){
	    ...
	    poollogin=params.login;
	    var payid = poollogin.split('.')[1];
	    var reallogin = poollogin.substr(0, poollogin.indexOf('.'));
	
	    if (isEmpty(payid)) { 
	    payid=null;
	    reallogin=params.login;
	    }
	    //if (!utils.isValidAddress(reallogin, config.poolServer.poolAddress)){   /* really anoying any one know whats wrong with base58 on my system */
            //    sendReply('invalid address used for login');
            //    return;
            //}

            var minerId = utils.uid();
            miner = new Miner(minerId, params.login, params.pass, ip,payid, portData.difficulty, pushMessage);
